...
from PyQt6.QtCore import (
    ...
    QTimer, QUrl, QTranslator, QLibraryInfo, QLocale, pyqtSignal,
)
...
MEMORY_CATEGORIES = {"identity": "Identidad", "preferences": "Preferencias", "projects": "Proyectos", "notes": "Notas", "relationships": "Relaciones", "facts": "Datos"}
...
class _DropCanvas(QWidget):
    ...
    def _paint_idle(self, p, W, H, hover):
        ...
        p.drawText(QRectF(0, cy + 8, W, 16), Qt.AlignmentFlag.AlignCenter,
                   "Arrastra un archivo o pulsa para elegir")
        p.setFont(QFont("Courier New", 7))
        p.setPen(QPen(qcol("#1a4a5a"), 1))
        p.drawText(QRectF(0, cy + 24, W, 14), Qt.AlignmentFlag.AlignCenter,
                   "Imágenes · Video · Audio · PDF · Código")
...
class CustomizeOverlay(QWidget):
    ...
    def __init__(self, assistant_name="JARVIS", user_name="",
        ...
        lay.addSpacing(4)
        lay.addWidget(_lbl("TU NOMBRE (opcional)", 8,
                            color=C.TEXT_DIM, align=Qt.AlignmentFlag.AlignLeft))
        self._user_input = QLineEdit(user_name)
        self._user_input.setPlaceholderText("Ej.: Daniel (opcional)")
        ...
        lay.addSpacing(4)
        clr_hdr = QHBoxLayout()
        clr_hdr.addWidget(_lbl("COLOR — arrastra el selector", 8,
                               color=C.TEXT_DIM, align=Qt.AlignmentFlag.AlignLeft))
        clr_hdr.addStretch()
        df_btn = QPushButton("PREDETERMINADO")
        df_btn.setFixedSize(108, 20)
        ...
        self._hex_input.setPlaceholderText("#00d4ff   (color hexadecimal)")
...
class PluginManagerOverlay(QWidget):
    ...
    def __init__(self, plugins: list[dict], parent=None):
        ...
        if not plugins:
            empty = QLabel("No se encontraron complementos.")
...
class AudioDeviceOverlay(_HudOverlay):
    ...
    def __init__(self, parent=None):
        ...
        def _row(label: str, kind: str, current: str) -> QComboBox:
            ...
            # host audio API.
            box.addItem("Predeterminado del sistema", "")
            ...
            if current and idx < 0:
                ...
                box.addItem(f"{current}  (desconectado)", current)
                box.setCurrentIndex(box.count() - 1)
            lay.addWidget(box)
            return box

        self._in_box  = _row("MICRÓFONO — entrada de voz",
                             "input", get_input_device())
        lay.addSpacing(4)
        self._out_box = _row("ALTAVOCES — salida de voz",
                             "output", get_output_device())

        note = QLabel("Al aplicar se reconecta la sesión y se conserva la conversación.")
...
class MemoryOverlay(_HudOverlay):
    ...
    def _rebuild(self):
        ...
        cap = QLabel(f"{len(rows)} recuerdos — más recientes primero. "
                     f"Los recuerdos se guardan en "
        ...
        if not rows:
            empty = QLabel("Todavía no hay recuerdos guardados.")
            ...
            for r in rows:
                ...
                meta = QLabel(f"{MEMORY_CATEGORIES.get(r['category'], r['category'])} · {r['updated'] or '—'}")
                ...
                rm.setToolTip("Olvidar este recuerdo")
                rm.setStyleSheet(f"""
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str):
        ...
        _cam_title = QLabel("◈  CÁMARA EN VIVO")
    ...
    def _create_desktop_shortcut(self):
        ...
        try:
            ...
            self._log.append_log("SYS: Acceso directo creado.")
        except Exception as e:
            self._log.append_log(f"ERR: No se pudo crear el acceso directo — {e}")
    ...
    def _build_right_panel(self) -> QWidget:
        ...
        self._file_hint = QLabel("Arrastra un archivo o pulsa arriba para cargarlo")
    ...
    def _build_quick_drawer(self) -> QWidget:
        ...
        sc_btn = QPushButton("⊞  ACCESO DIRECTO AL ESCRITORIO")
        ...
        self._autostart_btn = QPushButton("◉  INICIO AUTOMÁTICO: NO")
    ...
    def _build_footer(self) -> QWidget:
        ...
        lay.addWidget(_fl("[F2] Controles · [F4] Silenciar · [F11] Pantalla completa"))
        lay.addStretch()
    ...
    def _on_file_selected(self, path: str):
        ...
        self._file_hint.setText(f"{icon}  {p.name}  ·  {size}  ·  Indica a {self._assistant_name} qué hacer con el archivo")
    ...
    def _toggle_autostart(self):
        ...
        try:
            ...
            self._log.append_log(
                f"SYS: Inicio automático {'activado' if enabled else 'desactivado'}.")
        except Exception as e:
            self._log.append_log(f"ERR: Falló el inicio automático — {e}")

    def _update_autostart_btn(self, enabled: bool):
        if not hasattr(self, '_autostart_btn'):
            return
        if enabled:
            self._autostart_btn.setText("◉  INICIO AUTOMÁTICO: SÍ")
            ...
            self._autostart_btn.setText("◉  INICIO AUTOMÁTICO: NO")
            self._autostart_btn.setStyleSheet(f"""
                QPushButton {{
                    background: transparent; color: {C.TEXT_DIM};
                    border: 1px solid {C.BORDER}; border-radius: 3px;
    ...
    def _update_brief_btn(self, enabled: bool):
        if not hasattr(self, '_brief_btn'):
            return
        if enabled:
            self._brief_btn.setText("☀  RESUMEN DIARIO: SÍ")
            ...
            self._brief_btn.setText("☀  RESUMEN DIARIO: NO")
            self._brief_btn.setStyleSheet(f"""
                QPushButton {{
                    background: transparent; color: {C.TEXT_DIM};
                    border: 1px solid {C.BORDER}; border-radius: 3px;
    ...
    def _apply_name_update(self, name: str, user_name: str, ui_color: str = "",
        ...
        if display in ("JARVIS", "J.A.R.V.I.S"):
            self._sub_lbl.setText("Sistema de asistencia inteligente")
        else:
        ...
        try:
            ...
            if color_changed:
                self._log.append_log(f"SYS: Color aplicado — {ui_color}")
            if voice_changed:
                self._log.append_log(f"SYS: Voz seleccionada — {voice}")
        except Exception as e:
            self._log.append_log(f"ERR: No se pudo guardar la configuración — {e}")
