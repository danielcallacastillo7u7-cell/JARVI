...
def _capture_camera() -> tuple[bytes, str]:
    from core.face_tracking import camera_frame
    shared = camera_frame()
    if shared is not None:
        ok, encoded = cv2.imencode(".jpg", shared, [cv2.IMWRITE_JPEG_QUALITY, _JPEG_Q])
        if not ok:
            raise RuntimeError("No se pudo preparar la imagen de la cámara.")
