...
TOOL_DECLARATIONS = [
    {
        "name": "read_facial_expression",
        "description": "Read fresh local facial-mesh observations when the user asks about their gesture or expression. Reports visible movements only, never emotions, intentions, personality or health. Do not invent a reading when tracking is paused or no face is detected.",
...
class JarvisLive:
    ...
    def _build_config(self) -> types.LiveConnectConfig:
        ...
        parts.append(
            "[SPOKEN LANGUAGE] Respond and speak in Spanish by default, "
            "with natural Spanish pronunciation. Use Spanish for greetings, "
            "briefings, errors and alerts, regardless of older language memories "
            "or English tool results. Only switch languages if the user explicitly asks."
        )
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        try:
            if name == "read_facial_expression":
                from core.face_tracking import describe_expression
    ...
    async def _send_startup_briefing(self) -> None:
        ...
        lang_clause = (f" Speak this greeting in {lang}, then follow the "
                       f"user's explicit requests to change language."
                       if lang else "")
        ...
        async def _deliver_news():
            try:
                lang_str = (f" Speak in {lang} unless the user has since "
                            f"explicitly requested another language, in which case use it."
                            if lang else "")
